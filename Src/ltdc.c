  ...
  else
  {
		...
		for(index=0; index < height; index++) //Передвигаемся по оси y
		{
			...
			if(HAL_DMA2D_Init(&hdma2d) == HAL_OK)
			{
				if(HAL_DMA2D_ConfigLayer(&hdma2d, 1) == HAL_OK)
				{
					if (HAL_DMA2D_Start(&hdma2d, (uint32_t) pbmp, (uint32_t)memaddr, width, 1) == HAL_OK)
						{
							HAL_DMA2D_PollForTransfer(&hdma2d, 10);
						}
				}
...
void DMA2D_LayersAlphaReconfig(uint8_t alpha1, uint8_t alpha2)//Смена прозрачности в слоях 1 и 2
{
